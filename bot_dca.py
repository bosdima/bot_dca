...
Поддержка ступенчатого DCA с настраиваемой лестницей цен
"""
...
class Database:
    ...
    def reindex_purchases(self):
        ...
        try:
            ...
            cursor.execute("SELECT symbol, amount_usdt, price, quantity, multiplier, drop_percent, step_level, date FROM dca_purchases ORDER BY date ASC")
    ...
    def init_ladder_settings(self, symbol: str) -> Dict:
        ...
        # Если start_price не задан, пробуем получить текущую цену (позже)
    ...
    def save_ladder_settings(self, settings: Dict):
        ...
        try:
            ...
            cursor.execute('''
                ...
                (id, symbol, start_price, step_percent, steps_count, multiplier, base_amount, current_step, last_step_price)
                VALUES (
                    COALESCE((SELECT id FROM ladder_settings WHERE symbol = ?), ?),
                    ?, ?, ?, ?, ?, ?, ?, ?
                )
            ''', (
                settings['symbol'],
...
class DCAStrategy:
    ...
    async def execute_ladder_purchase(self, symbol: str, profit_percent: float) -> Dict:
        ...
        if result['success']:
            ...
            self.db.log_action(
                ...
                f"(цена {format_price(result['price'])} / целевая {format_price(target_price)})"
            )
    ...
    async def execute_dca_purchase(self, symbol: str, base_amount: float, 
        ...
        """Выполнить DCA покупку (старый метод - для совместимости)"""
    ...
    async def get_recommended_purchase(self, symbol: str) -> Dict:
        ...
        на основе текущей цены и настроек лестницы
        """
        current_price = await self.bybit.get_symbol_price(symbol)
        if not current_price:
            return {
...
class FastDCABot:
    ...
    async def show_dca_stats_detailed(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            # Добавляем информацию о лестнице
    ...
    async def show_status(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        symbol = self.db.get_setting('symbol', 'TONUSDT')
        is_active = self.db.get_setting('dca_active', 'false') == 'true'
        ladder_mode = self.db.get_setting('ladder_mode', 'true') == 'true'
        
        message = f"📋 *Статус бота*\n\n"
        message += f"🤖 Статус: {'✅ Активен' if is_active else '⏹ Остановлен'}\n"
        message += f"🪜 Режим: {'Мартингейл лесенка' if ladder_mode else 'Классический DCA'}\n"
    ...
    async def toggle_dca(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if is_active:
            ...
            if ladder_settings['start_price'] <= 0:
                # Автоматически устанавливаем стартовую цену
                ...
                await update.message.reply_text(
                    f"🪜 Автоматически установлена стартовая цена лестницы: {format_price(current_price, 4)} USDT"
                )
    ...
    async def set_ladder_start_price_save(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            await update.message.reply_text("❌ Некорректная цена. Введите число (например: 2.5):", reply_markup=self.get_cancel_keyboard())
            return SET_LADDER_START_PRICE
    ...
    async def set_ladder_step_percent_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text(
            ...
            f"*Пример:* 5 (каждый следующий шаг на 5% ниже)\n\n"
    ...
    async def set_ladder_multiplier_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text(
            ...
            f"*Пример:* 2 (каждый следующий шаг в 2 раза больше)\n\n"
    ...
    async def set_ladder_base_amount_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text(
            ...
            f"*Пример:* 1 (первая покупка на 1 USDT, вторая на {current_ladder['multiplier']} USDT и т.д.)\n\n"
            f"*Минимум:* 1 USDT",
            reply_markup=self.get_cancel_keyboard(),
            parse_mode='Markdown'
        )
    ...
    # ============= РУЧНАЯ ПОКУПКА С РЕКОМЕНДАЦИЯМИ =============
    ...
    async def manual_add_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Начать ручное добавление покупки с рекомендацией"""
    ...
    async def _validate_and_set_symbol(self, update: Update, symbol: str) -> int:
        ...
        # Если токен изменился, обновляем настройки лестницы
        if old_symbol != symbol:
            self.db.set_setting('last_purchase_price', '0')
            self.db.set_setting('last_purchase_time', '0')
            # Инициализируем лестницу для нового токена
            ladder = self.db.get_ladder_settings(symbol)
        ...
        await update.message.reply_text(
            ...
            f"🪜 Стартовая цена лестницы: {format_price(price, 4)} USDT",
            reply_markup=self.get_settings_keyboard()
        )
        return SELECTING_ACTION
    ...
    async def scheduler_loop(self):
        ...
        while self.scheduler_running:
            try:
                ...
                # Проверяем необходимость покупки по лестнице
                current_price = await self.bybit.get_symbol_price(symbol)
                if current_price:
                    ladder_info = self.db.calculate_ladder_purchase(current_price, symbol)
                    ...
                    if ladder_info['should_buy']:
                        ...
                        if result['success']:
                            ...
                            # Отправляем уведомление в Telegram (если есть активный чат)
                            if hasattr(self, 'application') and self.application.bot:
                                try:
                                    await self.application.bot.send_message(
                            ...
                            logger.warning(f"Ladder purchase failed: {result}")
