...
class Database:
    ...
    def init_db(self):
        ...
        try:
            ...
            defaults = [
                ('symbol', 'TONUSDT'),  # Изменено с BTCUSDT на TONUSDT
                ('invest_amount', '1'),
                ('profit_percent', '5'),
                ('max_drop_percent', '60'),
                ('max_multiplier', '3'),
    ...
    def import_database(self, file_path: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            for purchase in data.get('purchases', []):
                ...
                ''', (
                    purchase.get('id'),
                    purchase.get('symbol', 'TONUSDT'),  # По умолчанию TONUSDT
            ...
            for order in data.get('sell_orders', []):
                try:
                    ...
                    ''', (
                        order.get('id'),
                        order.get('symbol', 'TONUSDT'),  # По умолчанию TONUSDT
...
class FastDCABot:
    ...
    async def show_portfolio(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            symbol = self.db.get_setting('symbol', 'TONUSDT')  # По умолчанию TONUSDT
    ...
    async def set_symbol_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text(
            f"🪙 Введите символ (текущий: {self.db.get_setting('symbol', 'TONUSDT')}, или нажмите Отмена):",
            reply_markup=self.get_cancel_keyboard()
        )
        return SET_SYMBOL
